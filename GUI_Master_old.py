...
image2 =Image.open("S:/Project Code latest - Copy/Project Code latest - Copy/s1.jpg")
...
def openimage():
    ...
    testing_folder_path = "S:/Project Code latest - Copy/Project Code latest - Copy/testing"
...
#     # Check if the filename format is correct (contains a number in parentheses)
#     if not any(char.isdigit() for char in fn) or '(' not in fn or ')' not in fn:
#         update_label("Invalid image format. Please select a valid blood cell image.")
#         return
...
#     # Highlight detected cells only if the filename is of a blood cell image
#     if 'blood cell' in fn.lower():  # Modify this condition as needed based on the filename pattern
#         contours, _ = cv2.findContours(threshold, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
#         highlighted_img = img.copy()  # Initialize with a copy of the original image
#         highlighted_area = 0
#         for contour in contours:
#             area = cv2.contourArea(contour)
#             if area > 1000:  # Adjust this threshold as needed for sickle cells
#                 x, y, w, h = cv2.boundingRect(contour)
#                 cv2.rectangle(highlighted_img, (x, y), (x + w, y + h), (255, 0, 0), 2)  # Highlight with a blue border
#                 highlighted_area += area

#         im_highlighted = Image.fromarray(highlighted_img)
#         imgtk_highlighted = ImageTk.PhotoImage(image=im_highlighted)

#         img4 = tk.Label(root, text='Highlighted', font=('times new roman', 20, 'bold'), image=imgtk_highlighted,
#                          compound='bottom', height=250, width=250, bg='white')
#         img4.image = imgtk_highlighted
#         img4.place(x=1180, y=100)

#         total_area = x1 * y1
#         if highlighted_area > 0:
#             percentage_detected = (highlighted_area / total_area) * 100
#             result_label = tk.Label(root, text='Detected Cell Percentage: {:.2f}%'.format(percentage_detected),
#                                     font=('times new roman', 16, 'bold'), bg='white')
#         else:
#             result_label = tk.Label(root, text='No Cells Detected', font=('times new roman', 16, 'bold'), bg='white')

#         result_label.place(x=1180, y=360)
